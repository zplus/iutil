[demo](http://jsbin.com/dorexa/18/edit?html,css,js,output)
```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8"/>
    <meta name="viewport"
          content="width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=1, user-scalable=no"/>
    <title>拖拽</title>
    <style type="text/css">
        body { margin: 0; }
        * { -webkit-box-sizing: border-box; }
        .wrap { position: relative; float: left; color: #000; margin: 50px; width: 202px; height: 202px; overflow: hidden; background-color: #eee; border: 1px solid #000; }
        .j_drag { line-height: 50px; text-align: center; font-size: 40px; width: 50px; height: 50px; position: absolute; cursor: move; opacity: .5; border: 1px solid #333; }
    </style>
</head>
<body>
<div id="j_box1" class="wrap">
    <div class="j_drag" data-belong="#j_box1">1</div>
    <div class="j_drag" data-belong="#j_box2">2</div>
    <div class="j_drag" data-belong="#j_box3">3</div>
</div>
<div id="j_box2" class="wrap"></div>
<div id="j_box3" class="wrap"></div>
<script type="text/javascript">
    (function () {
        var oDrags = document.querySelectorAll('.j_drag');
        Array.prototype.forEach.call(oDrags, function (el) {
            el.style.backgroundColor = '#' + Math.floor(Math.random() * (parseInt("0xffffff", 16).toString(10))).toString(16);
            el.style.position = 'absolute';

            var parent = document.querySelector(el.getAttribute('data-belong'));
            if (!parent) return;
            parent.style.position = 'static';
            console.log(parent);

            var disX = 0;
            var disY = 0;

            var bounded = {
                top: parent.offsetTop,
                bottom: parent.clientHeight - el.offsetHeight + parent.offsetTop,
                left: parent.offsetLeft,
                right: parent.clientWidth - el.offsetWidth + parent.offsetLeft
            };
            console.log(bounded.x, bounded.y);

            el.onmousedown = function (event) {
                var e = event || window.event;
                disX = e.clientX - el.offsetLeft;
                disY = e.clientY - el.offsetTop;

                // IE8之前的版本，选中有bug
                // setCapture：全局捕获（就是生成了一个透明的层）
                el.setCapture && el.setCapture();

                document.onmousemove = function (event) {
                    var e = event || window.event;
                    var position = setPosition(e.clientX - disX, e.clientY - disY);
                    el.style.left = position.left + 'px';
                    el.style.top = position.top + 'px';
                };

                document.onmouseup = function () {
                    document.onmousemove = null;
                    document.onmouseup = null;
                    el.releaseCapture && el.releaseCapture();
                };

                return false;
            };

            function setPosition(curLeft, curTop) {
                var p = {};
                if (curLeft < bounded.left) {
                    p.left = bounded.left;
                } else if (curLeft > bounded.right) {
                    p.left = bounded.right;
                } else {
                    p.left = curLeft;
                }

                if (curTop < bounded.top) {
                    p.top = bounded.top;
                } else if (curTop > bounded.bottom) {
                    p.top = bounded.bottom;
                } else {
                    p.top = curTop;
                }
                return p;
            }
        });
    })();
</script>
</body>
</html>

```
